...
export function ErrorState({
  ...
  return (
    <div role="alert" className="flex flex-col items-center gap-4 px-6 py-16 text-center">
      <p className={`max-w-xs text-[14px] leading-relaxed ${c.text}`}>{message}</p>
      {onRetry && (
        <button
          onClick={onRetry}
          className={`rounded-xl px-5 py-2.5 text-[13px] font-bold text-white transition-transform duration-300 hover:brightness-110 active:scale-95 ${c.btn}`}
        >
          {t('common.retry')}
        </button>
      )}
