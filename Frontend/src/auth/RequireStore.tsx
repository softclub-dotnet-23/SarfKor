...
export function RequireStore() {
  ...
  if (!hasRole('StorePartner') || storeId === null || storeIdIsStale) {
    // A platform Admin has no reason to own a store — that role runs the
    // platform console (stores/subscriptions/users/reference/audit, see
    // AdminConsoleLayout), not the storefront cabinet — so route them
