...
**Результат**: аккаунт `Admin` теперь гарантированно существует на любой машине, где заданы `Admin:Email`/`Admin:Password` (dev — user-secrets, прод — переменные окружения при деплое). Не закоммичено — ждёт подтверждения пользователя.
