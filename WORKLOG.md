...
## 2026-08-03 (продолжение) — Прод: StorePartner не мог войти + мобильный аудит трёх ролей

Пользователь дал две задачи по уже задеплоенной на Railway версии (`resourceful-reflection-production-2a58.up.railway.app`), с явным требованием проверять фактами с живого прода, а не рассуждениями, и с явным запретом изобретать обходной путь в коде, если причина окажется в конфигурации Railway.

### ПРОБЛЕМА 1 — StorePartner не мог войти в кабинет

Проверены по порядку, фактами с живого прода, все 4 версии, которые назвал пользователь:

1. **Pending-статус магазина блокирует владельца** — по коду не должен (только скрывает магазин от потребителей); подтверждено и фактом: ниже, в ходе этой же проверки, свежесозданный магазин (заведомо `Pending`) сразу дал владельцу полный доступ к кабинету. Гипотеза отклонена.
2. **`Cors:AllowedOrigins` не пропускает домен фронтенда** — фронтенд (`resourceful-reflection-production-2a58.up.railway.app`) и бэкенд (`sarfkor-production.up.railway.app`) оказались двумя РАЗНЫМИ Railway-доменами (домен бэкенда не был известен заранее — найден через скачивание минифицированного JS-бандла задеплоенного фронтенда и грep по `.railway.app`, так как `VITE_API_BASE_URL` зашивается в сборку на этапе `vite build` и снаружи не виден). Первая проверка CORS случайно ушла на домен ФРОНТЕНДА и дала `405` без CORS-заголовков — это могло быть неверно истолковано как «CORS сломан». Повторная проверка (`curl -X OPTIONS` с `Origin`/`Access-Control-Request-*`) на правильном домене бэкенда дала чистый `204` с `access-control-allow-origin`, эхом повторяющим Origin фронтенда. CORS настроен верно. Гипотеза отклонена.
3. **Аккаунт на проде отличается от локального** — декодирование JWT (`role`-claim) после логина показало **только `User`**, без `StorePartner`; `GET /api/me/stores` вернул пустой список. **Это и есть причина**: у аккаунта `akmalzodaardasher@gmail.com` на отдельной prod-базе не было ни одного магазина и роли `StorePartner` — не код обваливался при резолюции магазина (`RequireStore.tsx`), а резолвить было физически нечего.
4. **Расхождение dev/prod-конфигурации (user-secrets не работают в проде)** — не подтверждено: тот же бэкенд, тот же JWT — Admin-логин на этом же проде отрабатывал корректно, значит инфраструктура/подпись токена в порядке; проблема изолирована именно в данных одного аккаунта, не в окружении.

**Причина: не баг кода и не конфигурация Railway — на аккаунте, которым проверял пользователь, действительно не было магазина на прод-базе.** Никакого обходного пути в коде не вносилось — по прямому выбору пользователя ("Создайте магазин через API прямо сейчас") магазин создан живым вызовом `POST /api/stores` с этим же аккаунтом на проде.

**Проверено реальным браузером на самом деплое** (Playwright, не локально): чистый вход по `resourceful-reflection-production-2a58.up.railway.app/login` под `akmalzodaardasher@gmail.com` → приземление на `/admin`, на странице виден полноценный дашборд владельца (не онбординг и не редирект на логин). Однозначный положительный результат.

**Railway: менять нечего.** `Cors:AllowedOrigins` уже настроен верно для домена фронтенда — никаких переменных окружения трогать не нужно.

### ПРОБЛЕМА 2 — мобильная версия для всех ролей

Аудит через Playwright с эмуляцией реального экрана телефона (`devices['iPhone 13']`, а не узкое окно десктопа), против задеплоенного фронтенда. Сначала — только сбор находок со скриншотами (без правок), как и просил пользователь.

**Проверено**: StorePartner — дашборд (в т.ч. с открытым мобильным меню), касса, склад, поставки, маркетинг, сотрудники, отчёты, настройки, онбординг магазина; общие экраны — вход, регистрация, оба этапа восстановления пароля (запрос кода + ввод кода с новым паролем).
**Пропущено по прямому решению пользователя**: раздел Admin (модерация/справочники) — не было доступных Admin-креды под рукой на проде.
**Не покрыто отдельными скриншотами**: экран Кассира (POS/смена) отдельно от Владельца — учётная запись-кассир на проде требует подтверждения email, а сервис на проде отправляет письмо реально (не в лог, в отличие от локальной среды без Smtp-секретов, см. запись 2026-08-02), и без доступа ни к почтовому ящику, ни к БД прода нет способа прочитать код в разумное время (см. ниже про скорость самой отправки). Смягчающий факт: страница кассы — тот же самый компонент `PosPage`, что уже проверен как владелец (переполнений нет), а найденный ниже баг сайдбара исправлен на уровне CSS без привязки к роли — значит чинится он и для кассира тоже, просто не подтверждён отдельным скриншотом.

**Находка (без переполнений)**: горизонтальных переполнений (`scrollWidth > clientWidth`) не найдено ни на одном из проверенных экранов. Мелкие/пограничные тап-таргеты (иконка «Обновить» 28×28, кнопка «Открыть смену» 175×30, фильтры периода на отчётах ~30px высотой) — на 2-4px меньше рекомендуемых 32px, но не нечитаемы и не перекрыты — оставлены как есть, не в категории «реально сломано».

**Подтверждённый баг**: мобильное выдвижное меню (`<aside>` в `AdminLayout.tsx`) не имело внутренней прокрутки. На экране высотой 664px (iPhone 13) содержимое сайдбара (лого + 8 пунктов навигации + карточка «Смена» + кнопка «Выйти») занимало 693px — кнопка «Выйти» полностью пропадала за нижней границей экрана (`logoutButtonVisible: false`, замерено через `getBoundingClientRect()`), без какого-либо способа до неё докрутить. Чем больше пунктов меню видит роль (у владельца — 8 пунктов, у кассира по факту меньше — сайдбар фильтрует `ownerOnly`-пункты для `Cashier`), тем сильнее переполнение; у владельца — наихудший случай.

**Исправлено**: `Frontend/src/admin/AdminLayout.tsx` — в className `<aside>` добавлен `overflow-y-auto` (было: `fixed inset-y-0 ... flex w-[240px] shrink-0 flex-col ...` без какого-либо overflow-правила). Точечная правка в одну CSS-утилиту, вёрстка вокруг не тронута.

**Проверено**: локально (не на самом проде — правка не запушена и не задеплоена, см. ниже), реальным браузером с эмуляцией iPhone 13, через одноразовый тестовый StorePartner-аккаунт (создан, подтверждён и магазин заведён напрямую через локальную БД/API; аккаунт-строка `AspNetUsers` осталась в локальной dev-базе — не удалилась только она из-за `RESTRICT`-связи с `SecurityEvents`, роль и магазин удалены). До правки: `scrollHeight` сайдбара 713px против `clientHeight` 664px, кнопка «Выйти» вне видимой области (`logoutButtonVisibleBeforeScroll: false`). После правки: `overflowY: auto` подтверждён вычисленным стилем, докрутка сайдбара до конца (`aside.scrollTop = aside.scrollHeight`) делает кнопку «Выйти» полностью видимой (`logoutButtonVisibleAfterScroll: true`) — скриншот подтверждает визуально.

Это прямо пересекается с отдельной, ещё не начатой задачей пользователя (убрать `ShiftCard` из сайдбара `AdminLayout` в пользу панели ассистента, перенести смену в `PosPage.tsx`) — после переноса контента в сайдбаре станет меньше и переполнение станет менее вероятным, но сама возможность прокрутки, добавленная здесь, остаётся нужна как защита на будущее (данные меню могут вырасти снова) и не отменяется этой предстоящей правкой.

### Побочная находка (вне рамок обеих задач, но встретилась в процессе аудита) — очень медленная отправка email на проде

Скриншот этапа «код из письма» при регистрации не удавалось снять — экран висел на «Подождите…» дольше 170 секунд. Прямой `curl -X POST .../api/auth/register` на прод-бэкенд подтвердил: запрос действительно висит **дольше 120 секунд** и в итоге отвечает `200` (не таймаут, не 5хх — просто очень медленно). Локально тот же самый код с теми же самыми SMTP-секретами (`Smtp:Username`/`Smtp:Password`, реальный Gmail) отправляет письмо и отвечает за **~3 секунды**. Код (`Infrastructure/Email/SmtpEmailSender.cs`) в обоих случаях идентичен и синхронно ждёт `ConnectAsync → AuthenticateAsync → SendAsync` внутри самого HTTP-запроса — разница только в окружении, не в коде. Это сильно указывает на медленное/нестабильное исходящее соединение к `smtp.gmail.com:587` именно с сети Railway (многие PaaS замедляют или ограничивают исходящий SMTP) — не диагностировано до конца (нет доступа к сетевым логам Railway), но воспроизведено дважды. **Не в рамках ни одной из двух заявленных задач — не исправлялось**, но стоит знать: любой пользователь, регистрирующийся или сбрасывающий пароль на проде, сейчас реально ждёт около/больше двух минут ответа, что на телефоне неотличимо от зависшего приложения. Рекомендация на будущее (не сделано): либо явный короткий таймаут на `SmtpClient` с быстрым понятным сообщением об ошибке вместо долгого молчания, либо вынос отправки письма из тела HTTP-запроса в фон (не блокировать ответ клиенту), либо переход на HTTP-based транзакционный email-провайдер вместо прямого SMTP.

### Проверено

`dotnet test` — **374/374**, без изменений (правка только во Frontend). `npx tsc -b` — 0 ошибок. `npm run lint` (`oxlint`) — без новых предупреждений (только предсуществующие, не относящиеся к этой правке). Временные Playwright-скрипты аудита и пакет `playwright` (`npm install/uninstall --no-save`, как и в прошлых сессиях) удалены после проверки, `package.json`/`package-lock.json` не тронуты.

### Что нужно сделать руками

1. **Ничего в переменных окружения Railway менять не нужно** для Проблемы 1 — CORS уже настроен верно, причиной было отсутствие магазина у конкретного аккаунта, что уже исправлено живым вызовом API.
2. **Правка сайдбара (`overflow-y-auto`) ещё не запушена** — она в рабочем дереве ветки `feature/ai-assistant` (как и весь остальной код этой сессии), не закоммичена сама по себе до конца этой записи. Ветка по-прежнему не смержена и не запушена, как и было изначально условлено — чтобы увидеть исправление на самом Railway, ветку нужно смержить и задеплоить.
3. Кассирский мобильный экран стоит один раз проверить вручную (или дать доступ к тестовому email-ящику/прод-БД) — не покрыт скриншотом в этом аудите по причине, описанной выше.
4. Стоит обратить внимание на скорость отправки email на проде (раздел выше) — не блокирует ничего из двух заявленных задач, но напрямую бьёт по регистрации/сбросу пароля реальных пользователей.
