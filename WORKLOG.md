...
**Результат**: поведение сохранено побайтово идентичным для всех проверенных путей; ни один из 231 теста не задет переносом (контроллеры — чисто hosting-слой, бизнес-логика в Application не менялась). Не закоммичено — ждёт явного подтверждения пользователя, как и работа с предыдущего шага (StoreCredit/ProductBundle/CustomerId).
